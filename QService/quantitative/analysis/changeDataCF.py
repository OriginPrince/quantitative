...
def change_final(request):
    if request.POST:
        ...
        try:
            ...
            cur = conn.cursor()
            #插入到计划表中
            sql1 = "insert into valueplan (ultimateInvest,rateInvest,terms,dayCount,id_user,class,time,alterms,volume,alInvest,over,rateTax) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            n = cur.execute(sql1, (ulInvest, inRate, terms, dayCount,id_user,tempClass,tempTime,tempAlterms,tempVolume,tempAlInvest,tempover,taxRate))
            conn.commit()
            #获取计划的id
            ...
            conn.close()
            print id_plan
            threadRE = threadRecord(inRate, terms, ulInvest, dayCount, id_plan,taxRate)
            # 调用子线程来执行定时任务，定投
            threadRE.start()
        except:
