...
def display_hist_zxb_price(request):
    if 'date' in request.GET:
        try:
            ...
            cur = conn.cursor()
            # 3.编写sql
            sql = "SELECT date,open,close,low,high,volume,amount FROM zxb_hist_data where date=%s"
            # 4.执行sql命令
...
def display_hist_zxb(request):
    try:
        ...
        cur = conn.cursor()
        # 3.编写sql
        sql = "SELECT date,open,close,low,high,volume,amount FROM zxb_hist_data"
        # 4.执行sql命令
        # execute可执行数据库查询select和命令insert，delete，update三种命令(这三种命令需要commit()或rollback())
        cur.execute(sql)
        # 5.获取数据
