...
def display_now_hs300_price(request):
    try:
        ...
        cur = conn.cursor()
        # 3.编写sql
        sql = "SELECT time,price FROM hs300_now_data order by time desc limit 1"
        # 4.执行sql命令
...
def display_now_hs300(request):
    try:
        ...
        cur = conn.cursor()
        # 3.编写sql
        sql = "SELECT time,price,date FROM hs300_now_data"
        # 4.执行sql命令
        # execute可执行数据库查询select和命令insert，delete，update三种命令(这三种命令需要commit()或rollback())
        cur.execute(sql)
        # 5.获取数据
