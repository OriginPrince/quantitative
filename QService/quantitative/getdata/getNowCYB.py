...
class threadCYB(threading.Thread):
    ...
    def run(self):
        while self.__running.isSet():
            ...
            df[['open','price','high','low','volume','amount','date','time']].to_sql('cyb_now_data',engine,if_exists='append')
            print df  
            time.sleep(1)
...
#让自增的数据从0开始
def deleteAll_job():
    engine.execute("truncate table cyb_now_data")
