# coding=utf-8
import threading
import tushare as ts
from sqlalchemy import create_engine
import time
from apscheduler.schedulers.background import BackgroundScheduler
from Queue import Queue

queue = Queue(30)
timer_interval=1
engine=create_engine('mysql://root:888212@127.0.0.1/economic?charset=utf8')

class StoreThread(threading.Thread):
    def run(self):
        global queue
        while True:
            df = queue.get()
            queue.task_done()
            df[['open', 'price', 'high', 'low', 'volume', 'amount', 'date', 'time','stock']].to_sql('nowdata', engine,
                                                                                            if_exists='append')

class threadZXB(threading.Thread):

    def __init__(self):
        threading.Thread.__init__(self)
        self.__flag = threading.Event()     # 用于暂停线程的标识
        self.__flag.clear()       # 设置为false
        self.__running = threading.Event()      # 用于停止线程的标识
        self.__running.set()      # 将running设置为True

    def run(self):
        while self.__running.isSet():
            self.__flag.wait()      # 为True时立即返回, 为False时阻塞直到内部的标识位为True后返回
            global queue
            df=ts.get_realtime_quotes('zxb')
            df['stock']='zxb'
            queue.put(df)
            time.sleep(1)

    def pause(self):
        self.__flag.clear()     # 设置为False, 让线程阻塞

    def resume(self):
        self.__flag.set()    # 设置为True, 让线程停止阻塞

    def stop(self):
        self.__flag.set()       # 将线程从暂停状态恢复, 如何已经暂停的话
        self.__running.clear()

class threadSZ50(threading.Thread):

    def __init__(self):
        threading.Thread.__init__(self)
        self.__flag = threading.Event()     # 用于暂停线程的标识
        self.__flag.clear()       # 设置为false
        self.__running = threading.Event()      # 用于停止线程的标识
        self.__running.set()      # 将running设置为True

    def run(self):
        while self.__running.isSet():
            self.__flag.wait()      # 为True时立即返回, 为False时阻塞直到内部的标识位为True后返回
            global queue
            df=ts.get_realtime_quotes('sz50')
            df['stock'] = 'sz50'
            queue.put(df)
            time.sleep(1)

    def pause(self):
        self.__flag.clear()     # 设置为False, 让线程阻塞

    def resume(self):
        self.__flag.set()    # 设置为True, 让线程停止阻塞

    def stop(self):
        self.__flag.set()       # 将线程从暂停状态恢复, 如何已经暂停的话
        self.__running.clear()

class threadSZ(threading.Thread):

    def __init__(self):
        threading.Thread.__init__(self)
        self.__flag = threading.Event()     # 用于暂停线程的标识
        self.__flag.clear()       # 设置为false
        self.__running = threading.Event()      # 用于停止线程的标识
        self.__running.set()      # 将running设置为True

    def run(self):
        while self.__running.isSet():
            self.__flag.wait()      # 为True时立即返回, 为False时阻塞直到内部的标识位为True后返回
            global queue
            df=ts.get_realtime_quotes('sz')
            df['stock'] = 'sz'
            queue.put(df)
            time.sleep(1)

    def pause(self):
        self.__flag.clear()     # 设置为False, 让线程阻塞

    def resume(self):
        self.__flag.set()    # 设置为True, 让线程停止阻塞

    def stop(self):
        self.__flag.set()       # 将线程从暂停状态恢复, 如何已经暂停的话
        self.__running.clear()

class threadSH(threading.Thread):

    def __init__(self):
        threading.Thread.__init__(self)
        self.__flag = threading.Event()     # 用于暂停线程的标识
        self.__flag.clear()       # 设置为false
        self.__running = threading.Event()      # 用于停止线程的标识
        self.__running.set()      # 将running设置为True

    def run(self):
        while self.__running.isSet():
            self.__flag.wait()      # 为True时立即返回, 为False时阻塞直到内部的标识位为True后返回
            global queue
            df=ts.get_realtime_quotes('sh')
            df['stock'] = 'sh'
            queue.put(df)
            time.sleep(1)

    def pause(self):
        self.__flag.clear()     # 设置为False, 让线程阻塞

    def resume(self):
        self.__flag.set()    # 设置为True, 让线程停止阻塞

    def stop(self):
        self.__flag.set()       # 将线程从暂停状态恢复, 如何已经暂停的话
        self.__running.clear()

class threadHS300(threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
        self.__flag = threading.Event()     # 用于暂停线程的标识
        self.__flag.clear()       # 设置为false
        self.__running = threading.Event()      # 用于停止线程的标识
        self.__running.set()      # 将running设置为True

    def run(self):
        while self.__running.isSet():
            self.__flag.wait()      # 为True时立即返回, 为False时阻塞直到内部的标识位为True后返回
            global queue
            df=ts.get_realtime_quotes('hs300')
            df['stock'] = 'hs300'
            queue.put(df)
            time.sleep(1)

    def pause(self):
        self.__flag.clear()     # 设置为False, 让线程阻塞

    def resume(self):
        self.__flag.set()    # 设置为True, 让线程停止阻塞

    def stop(self):
        self.__flag.set()       # 将线程从暂停状态恢复, 如何已经暂停的话
        self.__running.clear()
		
class threadCYB(threading.Thread):
    def __init__(self):
        threading.Thread.__init__(self)
        self.__flag = threading.Event()     # 用于暂停线程的标识
        self.__flag.clear()       # 设置为False,当start线程的时候并不运行，resume的时候才去运行，这样的话当脚本启动的时候不会立即执行
        self.__running = threading.Event()      # 用于停止线程的标识
        self.__running.set()      # 将running设置为True

    def run(self):
        while self.__running.isSet():
            self.__flag.wait()      # 为True时立即返回, 为False时阻塞直到内部的标识位为True后返回
            global queue
            df=ts.get_realtime_quotes('cyb')
            df['stock'] = 'cyb'
            queue.put(df)
            time.sleep(1)

    def pause(self):
        self.__flag.clear()     # 设置为False, 让线程阻塞

    def resume(self):
        self.__flag.set()    # 设置为True, 让线程停止阻塞

    def stop(self):
        self.__flag.set()       # 将线程从暂停状态恢复, 如何已经暂停的话
        self.__running.clear()
			     

threadcyb = threadCYB()
threadsh = threadSH()
threadsz = threadSZ()
threadsz50 = threadSZ50()
threadzxb = threadZXB()
threadhs300 = threadHS300()

def start_job():
    threadcyb.start()
    threadsh.start()
    threadsz.start()
    threadsz50.start()
    threadzxb.start()
    threadhs300.start()
    print("start")	
    
def stop_job():
    threadcyb.stop()
    threadsh.stop()
    threadsz.stop()
    threadsz50.stop()
    threadzxb.stop()
    threadhs300.stop()
    
def pause_morning_job():
    threadcyb.pause()
    threadsh.pause()
    threadsz.pause()
    threadsz50.pause()
    threadzxb.pause()
    threadhs300.pause()
    print("pause morning")
	
def pause_afternoon_job():
    threadcyb.pause()
    threadsh.pause()
    threadsz.pause()
    threadsz50.pause()
    threadzxb.pause()
    threadhs300.pause()
    print("pause morning")
	
def resume_morning_job():
    threadcyb.resume()
    threadsh.resume()
    threadsz.resume()
    threadsz50.resume()
    threadzxb.resume()
    threadhs300.resume()
    print("resume morning")

def resume_afternoon_job():
    threadcyb.resume()
    threadsh.resume()
    threadsz.resume()
    threadsz50.resume()
    threadzxb.resume()
    threadhs300.resume()
    print("resume morning")

#让自增的数据从0开始
def deleteAll_job():
    engine.execute("truncate table nowdata")
    print("truncate table")
   
if __name__=="__main__":
    start_job()
    scheduler = BackgroundScheduler()
    scheduler.add_job(resume_morning_job, 'cron', day_of_week='mon-fri', hour=9, minute=30)
    scheduler.add_job(pause_morning_job, 'cron', day_of_week='mon-fri', hour=11, minute=30)
    scheduler.add_job(resume_afternoon_job, 'cron', day_of_week='mon-fri', hour=13, minute=00)
    scheduler.add_job(pause_afternoon_job, 'cron', day_of_week='mon-fri', hour=15, minute=00)
    scheduler.add_job(deleteAll_job, 'cron', day_of_week='mon-fri', hour=9, minute=25)
    scheduler.start()
	
    t=time.strftime("%H:%M:%S",time.localtime(time.time()))
    day=time.strftime('%A')
    weekdays=['Monday','Tuesday','Wednesday','Thursday','Friday']
    flagDay=False
    if day in weekdays:
        flagDay=True
    if (t>'09:30:00' and t<'11:30:00' and flagDay is True) or (t>'13:00:00' and t<'15:00:00' and flagDay is True):
        resume_morning_job()
    StoreThread().start()
    flag=True

    while flag is True:
        time.sleep(2)
        if threadcyb.isAlive() and threadhs300.isAlive() and threadsz50.isAlive() and threadsh.isAlive() and threadsz.isAlive() and threadzxb.isAlive():
            pass
        else:
            flag = False

    if flag is False:
        exit()
    else:
        raise Exception("出现错误")

